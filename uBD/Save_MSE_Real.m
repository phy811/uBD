function Save_MSE_Real(combtype, R, m, nPts)
    %======================================================================
    ...
    %  R : bootstrap sample size (e.g. 5e4)
    %  m : #bootstrap replicates (e.g. 20)
    %  nPts : #epsilon grid points per interval (e.g. 50)
    %======================================================================
    ...
    epsRange = [0.1 epsL;   epsL epsH;   epsH 10];
    
    % Subsample data
    rng(1,'twister');
    subsampIdx = zeros(m, R);   % m×R
    for r = 1:m
        subsampIdx(r, :) = randperm(nTotal, R);
    end
    ...
    for s = 1:3
        ...
        for k = 1:nPts
            ...
            fprintf('[%s] seg %d/3, eps %d/%d (%.4f) done\n', ...
                combtype, s, k, nPts, epsList(k));
        end
    end
    
    % Save MSE
    outfile = sprintf('MSE_%s_R%d_m%d_nPts%d.mat',lower(combtype),R,m,nPts);
    save(outfile,...
          'epsRange','nPts', ...
